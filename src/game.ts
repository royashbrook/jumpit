import { LEVELS, REGIONS, TILE, type Level, type TerrainKind } from './levels.ts'
import {
  createSimulation,
  guardianState,
  stepSimulation,
...
export type GameCue = SimulationCue | 'controls-learned' | 'pause'
export interface GameState {
  ...
  respawning: boolean
  message: string
  hiddenLightId: string | null
  guardianHealth: number
  guardianMax: number
  guardianDefeated: boolean
}
export interface Game {
  start(levelId?: string, options?: { foundHiddenLights?: string[] }): void
  restart(): void
  stop(): void
  destroy(): void
  resize(): void
  setInput(action: GameAction, pressed: boolean, source?: string): void
  togglePause(): boolean
  pause(): boolean
  clearInput(): void
}
interface InputState { left: boolean; right: boolean; jumpHeld: boolean; jumpPressed: boolean }
interface CoachState { moved: boolean; jumped: boolean; glowing: boolean; sparkFrames?: number; x: number; frame?: number }
interface Particle { x: number; y: number; vx: number; vy: number; life: number; color: string }
interface Impact { x: number; y: number; color: string; life: number; maxLife: number; expands: boolean }
...
export function cameraScale(width: number, height: number) {
...
export function cameraTarget({ playerX, direction = 0, viewWidth, worldWidth, reducedMotion = false }: {
  playerX: number; direction?: number; viewWidth: number; worldWidth: number; reducedMotion?: boolean
}) {
...
export function verticalCameraTarget({ playerY, playerHeight, viewHeight }: { playerY: number; playerHeight: number; viewHeight: number }) {
  return clamp(playerY + playerHeight - viewHeight * .7, 0, Math.max(0, WORLD_HEIGHT - viewHeight))
}

export function cameraProgress(camera: number, worldWidth: number, viewWidth: number) {
...
export function backgroundCrop({ imageWidth, rowHeight, width, height, progress }: {
  imageWidth: number; rowHeight: number; width: number; height: number; progress: number
}) {
...
type ArtKey = keyof typeof ART_SOURCES

export function artKeysForLevel(level: Level): ArtKey[] {
  const keys: ArtKey[] = ['courier']
...
export function coachMessage({ moved, jumped, glowing, sparkFrames = 0, x, frame = 0 }: CoachState) {
...
export function playHint({ finished, moved, jumped, glowing, sparkFrames = 0, x, y, finishX, finishY, frame = 0 }: CoachState & {
  finished: boolean; y: number; finishX: number; finishY: number
}) {
...
export function impactFeedback(type: string, reducedMotion = false) {
...
export function sparkGlowBright(sparkFrames: number, animationFrame: number) {
  return sparkFrames > 60 || (sparkFrames > 0 && Math.floor(animationFrame / 6) % 2 === 0)
}

export function drawPowerAura(context: CanvasRenderingContext2D, subject: Pick<Player, 'glowing' | 'sparkFrames'>, animationFrame: number, width: number, height: number) {
...
export function terrainVisible(rect: Pick<WorldTerrain, 'x' | 'y' | 'w' | 'h'>, cameraLeft: number, cameraTop: number, viewWidth: number, viewHeight: number) {
  const margin = TILE * 2
...
export function keyInputMode({ type, running, paused, finished, respawning = false }: {
  type: string; running: boolean; paused: boolean; finished: boolean; respawning?: boolean
}) {
...
export function interactiveKeyTarget(target: unknown, code: string) {
  return code === 'Space' && Boolean(target && typeof target === 'object' && 'closest' in target &&
    typeof target.closest === 'function' && target.closest('button, a[href], input, select, textarea, summary, [contenteditable]'))
}

export function setInputState(input: InputState, action: GameAction, pressed: boolean) {
  if (action === 'jump') {
...
export function clearInputState(input: InputState, player: Pick<Player, 'jumpBuffer'>) {
  input.left = false
  input.right = false
...
export function createGame(canvas: HTMLCanvasElement, onState: (state: GameState) => void = () => {}, onCue: (cue: GameCue) => void = () => {}): Game {
  const drawingContext = canvas.getContext('2d')
  if (!drawingContext) throw new Error('A 2D canvas context is required')
  const context = drawingContext
  const art = {
    gardenBackground: { source: ART_SOURCES.gardenBackground, image: new Image() },
    regionBackground: { source: ART_SOURCES.regionBackground, image: new Image() },
    finalBackground: { source: ART_SOURCES.finalBackground, image: new Image() },
    courier: { source: ART_SOURCES.courier, image: new Image() },
    world: { source: ART_SOURCES.world, image: new Image() },
    region: { source: ART_SOURCES.region, image: new Image() },
    final: { source: ART_SOURCES.final, image: new Image() },
  }
  ...
  let cameraKick = 0
  let respawn: { life: number; player: Player | null } | null = null
  ...
  let accumulator = 0
  let raf = 0
  let knownHiddenLights: string[] = []
  let ran = false
  let controlsLearned = false

  function loadArt(level: Level) {
    const needed = new Set<string>(artKeysForLevel(level))
  ...
  function report(message = '', { hiddenLightId = null }: { hiddenLightId?: string | null } = {}) {
    if (destroyed) return
  ...
  function burst(x: number, y: number, color = '#FFD563') {
  ...
  function drawBackground(width: number, height: number, viewWidth: number) {
  ...
  function terrainPalette(kind: TerrainKind): [string, string, string] {
  ...
  function drawTerrain(rect: WorldTerrain) {
  ...
  function drawSeed(seed: Collectible) {
  ...
  function drawCell(sheet: HTMLImageElement, cell: number, x: number, y: number, width: number, height: number, flip = 1) {
  ...
  const drawWorldCell = (cell: number, x: number, y: number, width: number, height: number, flip = 1) =>
  ...
  function drawHint(width: number, height: number) {
    const hint = playHint({
      finished,
      moved,
      jumped,
  ...
  function drawRespawn(width: number, height: number) {
    if (!respawn) return
  ...
  function loop(time: number) {
    ...
    lastTime = time
    while (accumulator >= FIXED_STEP && running && !destroyed) {
      update()
      accumulator -= FIXED_STEP
    }
    if (!running || destroyed) return
    paint()
    if (running && !destroyed) raf = requestAnimationFrame(loop)
  }

  function setInput(action: GameAction, pressed: boolean, source = 'direct') {
    if (destroyed) return
  ...
  function destroy() {
    ...
    for (const { image } of Object.values(art)) {
