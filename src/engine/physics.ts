...
export interface Terrain extends Rect {
  type?: 'oneway' | 'solid'
}
...
export type PhysicsConfig = { -readonly [Key in keyof typeof DEFAULTS]: number }
...
const overlap = (body: Rect, rect: Rect) =>
  body.x < rect.x + rect.w &&
  body.x + body.w > rect.x &&
  body.y < rect.y + rect.h &&
  body.y + body.h > rect.y

export function createBody({ x = 0, y = 0, ...tuning }: Partial<PhysicsConfig> & { x?: number, y?: number } = {}): Body {
...
function moveHorizontal(body: Body, amount: number, terrain: readonly Terrain[]) {
...
function moveVertical(body: Body, amount: number, terrain: readonly Terrain[], direction: number) {
  const steps = Math.max(1, Math.ceil(Math.abs(amount) / 8))
  const step = amount / steps
  body.onGround = false
  for (let index = 0; index < steps; index += 1) {
...
export function stepPhysics(body: Body, input: PhysicsInput = {}, terrain: readonly Terrain[] = []) {
...
export function isStomp(player: Pick<Body, 'y' | 'h' | 'vy'>, target: Pick<Rect, 'y'>) {
  const feet = player.y + player.h
  return player.vy > 0 && feet + player.vy >= target.y && feet <= target.y + Math.max(12, player.vy + 4)
}

export function isSideDamage(player: Rect & Pick<Body, 'vy'>, target: Rect) {
