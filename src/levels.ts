export type RegionId = 'garden' | 'rooftop' | 'workshop' | 'market' | 'keep'
export type TerrainKind = 'ground' | 'leaf' | 'crumble' | 'roof' | 'slick' | 'awning'
  | 'wood' | 'belt' | 'lift' | 'bridge' | 'stall' | 'lantern' | 'shade' | 'stone' | 'beacon'
export type EnemyKind = 'mossling' | 'drizzlet' | 'gearling' | 'mothlight' | 'sentry' | 'warden'
export type ObjectKind = EnemyKind | 'seed' | 'checkpoint' | 'spring' | 'fan' | 'switch'
  | 'lamp' | 'gate' | 'cloak' | 'hidden-light'
export type Point = [x: number, y: number]
export type TerrainTuple = [id: string, kind: TerrainKind, x: number, y: number, width: number, height: number]
export type ObjectTuple<Kind extends ObjectKind = ObjectKind> = [id: string, kind: Kind, x: number, y: number]
export type FinishTuple = [id: string, x: number, y: number]
...
export interface Level {
  ...
  finish: FinishTuple
  introduces: string[]
  terrain: TerrainTuple[]
  objects: ObjectTuple[]
}

interface TrailFamily {
...
export const REGIONS: readonly { id: RegionId, name: string }[] = Object.freeze([
...
const TRAIL_FAMILY = Object.freeze({
  hops: Object.freeze<TrailFamily>({
    terrain: [
  ...
  launch: Object.freeze<TrailFamily>({
    terrain: [
  ...
  motion: Object.freeze<TrailFamily>({
    terrain: [
  ...
  perch: Object.freeze<TrailFamily>({
    terrain: [
...
const FAMILY_BY_LEVEL: Readonly<Record<string, keyof typeof TRAIL_FAMILY>> = Object.freeze({
...
function extendTrail(entry: Level): Level {
  ...
  const suffix = (index: number) => `${entry.id}-trail-${String.fromCharCode(97 + index)}`
  const highSteps: TrailFamily['terrain'] = entry.finish[2] === 8
    ? [[31, 12, 7, 1], [38, 10, 7, 1], [45, 9, 11, 1]]
    : [[31, 12, 7, 1], [38, 10, 7, 1], [45, 8, 7, 1], [50, 7, 6, 1]]
  const finalRoute: TrailFamily['terrain'] = [
  ...
  const route = finalKeep
    ...
    : highKeep
      ...
      : family.terrain
  const extension = route.map<TerrainTuple>(([x, y, span, height], index) => [
  ...
  const checkpoint: Point = finalKeep ? [50, 14] : [24, 14]
  let objects = entry.objects.map<ObjectTuple>(object => {
  ...
  objects = [
    ...
    ...(entry.objects.some(([, kind]) => kind === 'spring')
      ? [[`${entry.id}-trail-spring`, 'spring', origin + 7, 14] satisfies ObjectTuple]
      : []),
    ...(entry.objects.some(([, kind]) => kind === 'fan') && !highKeep
      ? [[`${entry.id}-trail-fan`, 'fan', origin + 36, 14] satisfies ObjectTuple]
      : []),
  ]

  const finish: FinishTuple = finalKeep
...
const level = (id: string, order: number, region: RegionId, name: string, width: number, spawn: Point, finish: FinishTuple, introduces: string[], terrain: TerrainTuple[], objects: ObjectTuple[]) => extendTrail({
...
const TERRAIN_KINDS = new Set<unknown>([
  'ground', 'leaf', 'crumble', 'roof', 'slick', 'awning', 'wood', 'belt',
  'lift', 'bridge', 'stall', 'lantern', 'shade', 'stone', 'beacon',
])
const OBJECT_KINDS = new Set<unknown>([
  'seed', 'checkpoint', 'mossling', 'spring', 'drizzlet', 'fan', 'gearling',
  'switch', 'mothlight', 'lamp', 'gate', 'sentry', 'warden', 'cloak', 'hidden-light',
])
const ENEMY_KINDS = new Set<unknown>(['mossling', 'drizzlet', 'gearling', 'mothlight', 'sentry', 'warden'])
type ValidationTerrain = [id: unknown, kind: unknown, x: number, y: number, width: number, height: number]
type ValidationObject = [id: unknown, kind: unknown, x: number, y: number]

const isArray = (value: unknown): value is unknown[] => Array.isArray(value)
const isRecord = (value: unknown): value is Record<string, unknown> =>
  Boolean(value) && typeof value === 'object' && !Array.isArray(value)
const integer = (value: unknown): value is number => Number.isInteger(value)
const pointInBounds = ([x, y]: readonly unknown[], [width, height]: readonly number[]) => integer(x) && integer(y) && x >= 0 && y >= 0 && x < width && y < height
const contains = (terrain: ValidationTerrain, x: number, y: number) => x >= terrain[2] && x < terrain[2] + terrain[4] && y >= terrain[3] && y < terrain[3] + terrain[5]
const supports = (terrain: ValidationTerrain, x: number, y: number) => x >= terrain[2] && x < terrain[2] + terrain[4] && y === terrain[3] - 1
const stableSupport = (terrain: ValidationTerrain, x: number, y: number) => !['crumble', 'lift', 'bridge'].some(kind => kind === terrain[1]) && supports(terrain, x, y)

function jumpable(from: ValidationTerrain, to: ValidationTerrain) {
...
function reachableTerrain(terrain: ValidationTerrain[], start: readonly number[]) {
...
function hasPit(terrain: ValidationTerrain[], height: number) {
  const floor = terrain
...
export function validateCampaign(campaign: unknown = LEVELS): string[] {
  const errors: string[] = []
  if (!isArray(campaign)) return ['campaign must be an array']
  if (campaign.length !== CAMPAIGN_ORDER.length) errors.push(`campaign must contain ${CAMPAIGN_ORDER.length} levels`)

  const ids = new Map<string, string>()
  const features = new Set<string>()
  const hiddenLightsByRegion = new Map<unknown, number>(REGIONS.map(region => [region.id, 0]))
  ...
  campaign.forEach((entry, index) => {
    ...
    const sizeOk = isArray(rawSize) && rawSize.length === 2 && rawSize.every(integer) && rawSize[0] > 0 && rawSize[1] > 0
    if (!sizeOk) errors.push(`${where}: size must be two positive integers`)
    const size = sizeOk ? rawSize : [0, 0]

    const spawn = entry.spawn
    const spawnOk = isArray(spawn) && spawn.length === 2 && spawn.every(integer) && pointInBounds(spawn, size)
    if (!spawnOk) errors.push(`${where}: spawn must be an in-bounds integer point`)

    const finish = entry.finish
    const finishPoint = isArray(finish) ? finish.slice(1) : []
    const finishOk = isArray(finish) && finish.length === 3 && finishPoint.every(integer) && pointInBounds(finishPoint, size)
    if (!finishOk) errors.push(`${where}: finish bell must be [id, x, y] in bounds`)
    if (isArray(entry.finish)) claim(entry.finish[0], `${where} finish`)

    if (!isArray(entry.introduces) || entry.introduces.length === 0) {
    ...
    const terrain = isArray(entry.terrain) ? entry.terrain : []
    if (!isArray(entry.terrain) || terrain.length < 2) errors.push(`${where}: terrain must contain at least two platforms`)
    const validTerrain: ValidationTerrain[] = []
    for (const [terrainIndex, tile] of terrain.entries()) {
      const tileWhere = `${where} terrain ${terrainIndex + 1}`
      if (!isArray(tile) || tile.length !== 6) {
      ...
      } else validTerrain.push([tile[0], tile[1], box[0], box[1], box[2], box[3]])
    }
    if (sizeOk && !hasPit(validTerrain, size[1])) errors.push(`${where}: terrain must include a pit`)

    const objects = isArray(entry.objects) ? entry.objects : []
    if (!isArray(entry.objects)) errors.push(`${where}: objects must be an array`)
    let seeds = 0
    let checkpoints = 0
    let enemies = 0
    const validObjects: ValidationObject[] = []
    for (const [objectIndex, object] of objects.entries()) {
      const objectWhere = `${where} object ${objectIndex + 1}`
      if (!isArray(object) || object.length !== 4) {
      ...
      const point = object.slice(2)
      if (!point.every(integer) || !pointInBounds(point, size)) errors.push(`${objectWhere}: point is out of bounds`)
      else validObjects.push([object[0], object[1], point[0], point[1]])
    ...
    for (const object of validObjects.filter(item => ['seed', 'checkpoint', 'hidden-light'].some(kind => kind === item[1]))) {
      if (validTerrain.some(tile => contains(tile, object[2], object[3]))) {
        errors.push(`${where}: ${object[0]} is embedded in terrain`)
      }
    }

    if (spawnOk && !validTerrain.some(tile => supports(tile, spawn[0], spawn[1]))) {
      errors.push(`${where}: spawn has no supporting terrain`)
    }
    if (finishOk && !validTerrain.some(tile => stableSupport(tile, finishPoint[0], finishPoint[1]))) {
    ...
    if (spawnOk) {
      const reachable = reachableTerrain(validTerrain, spawn)
      const reachablePoint = ([x, y]: readonly number[]) => validTerrain.some((tile, tileIndex) => reachable.has(tileIndex) && supports(tile, x, y))
      if (finishOk && !reachablePoint(finishPoint)) errors.push(`${where}: finish bell is outside the conservative jump envelope`)
      for (const object of validObjects.filter(item => ['seed', 'checkpoint', 'hidden-light'].some(kind => kind === item[1]))) {
        if (!reachablePoint([object[2], object[3]])) errors.push(`${where}: ${object[0]} is outside the conservative jump envelope`)
      }
    }
  })
...
export function assertCampaign(campaign?: undefined): Level[]
export function assertCampaign<Value>(campaign: Value): Value
export function assertCampaign(campaign: unknown = LEVELS) {
