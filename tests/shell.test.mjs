...
test('the worker keeps navigation network-first and the update probe uncached', async () => {
  ...
  assert.match(worker, /const CACHE = 'jumpit-v2\.0\.0'/)
...
test('the worker removes only old Jumpit caches', async () => {
  ...
  const sandbox = {
    ...
    caches: {
      keys: async () => ['jumpit-v0.9.0', 'jumpit-v1.5.0', 'jumpit-v1.8.0', 'jumpit-v1.9.0', 'jumpit-v2.0.0', 'sibling-game-v4'],
  ...
  await done
  assert.deepEqual(deleted, ['jumpit-v0.9.0', 'jumpit-v1.5.0', 'jumpit-v1.8.0', 'jumpit-v1.9.0'])
})
