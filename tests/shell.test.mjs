...
test('the worker keeps navigation network-first and the update probe uncached', async () => {
  ...
  assert.match(worker, /const CACHE = 'jumpit-v2\.0\.0-r15'/)
  assert.match(await text('index.html'), /app\.css\?v=10[\s\S]*app\.js\?v=13/)
  assert.match(await text('app.js'), /game\.js\?v=11[\s\S]*levels\.js\?v=2[\s\S]*save\.js\?v=2[\s\S]*update\.js\?v=4/)
...
test('an exact r12 cache-first controller cannot mix old gameplay into the r15 shell', async () => {
  ...
  assert.deepEqual([...current].sort(), [
    'app.js?v=13', 'engine/physics.js?v=2', 'engine/simulation.js?v=2',
    'game.js?v=11', 'levels.js?v=2', 'save.js?v=2',
  ])
  for (const url of current) {
    assert.equal(r12.has(url), false, `r12 can serve stale ${url}`)
    assert.match(worker, new RegExp(`['"]\\./${url.replace(/[.?]/g, '\\$&')}['"]`), `r15 does not precache ${url}`)
  }

  // If r15 claims before app code attaches controllerchange, no reload fires.
